...
def test_create_room_success(auth_room_service, room_payload):
    response = auth_room_service.create_room(room_payload)
...
def test_get_room_by_id(auth_room_service, room_payload):
    create_response = auth_room_service.create_room(room_payload)
    assert create_response.status_code == 201
    room_id = create_response.json()["roomid"]

    get_response = auth_room_service.get_room_by_id(room_id)
...
def test_get_all_rooms(auth_room_service, room_payload):
    create_response = auth_room_service.create_room(room_payload)
    assert create_response.status_code == 201

    get_all_response = auth_room_service.get_all_rooms()
    assert get_all_response.status_code == 200
    rooms = get_all_response.json()["rooms"]
    assert any(room["roomid"] == create_response.json()["roomid"] for room in rooms)

def test_get_room_by_id_not_found(room_service):
    response = room_service.get_room_by_id(-1)
    assert response.status_code == 404

@pytest.mark.parametrize("missing_field", ["roomName", "type", "roomPrice"])
def test_create_room_missing_required_fields(auth_room_service, room_payload, missing_field):
    invalid_payload = room_payload.copy()
    del invalid_payload[missing_field]

    response = auth_room_service.create_room(invalid_payload)
